package dev.matteomac81888.echo.extensions.cache
...
import dev.matteomac81888.echo.extensions.exceptions.AppException.Companion.toAppException
...
object Cached {
    ...
    // Il campo streamProvider di Streamable.Source.Raw non è serializzabile: dopo un
    // salvataggio/ripristino su disco torna sempre null, quindi una copia cache di questo
    // tipo non è mai realmente utilizzabile per la riproduzione e va scartata come fallback.
    private fun Streamable.Media.isUsableAfterCacheRestore(): Boolean = when (this) {
        is Streamable.Media.Server -> sources.none {
            it is Streamable.Source.Raw && it.streamProvider == null
        }
    ...
    suspend fun loadStreamableMedia(
        ...
        val media = extension.getAs<TrackClient, Streamable.Media> {
            ...
            fileCache.getData<Streamable.Media>(id).getOrNull()
                ?.takeIf { it.isUsableAfterCacheRestore() }
                ?: throw throwable
        }
