package dev.matteomac81888.echo.extensions.builtin.unified
...
import dev.matteomac81888.echo.playback.MediaItemUtils.toKey
...
class UnifiedExtension(
    ...
    override suspend fun loadLibraryFeed() = Feed(
        ...
        extension?.getFeedData<LibraryFeedClient> { loadLibraryFeed() } ?: run {
            PagedData.Single {
                ...
                // CRONOLOGIA MEMORY SAVER
                val historyTracks = db.playlistDao().getHistory(30).map { it.track }

                listOfNotNull(
                    if (historyTracks.isNotEmpty()) Shelf.Category(
                        "history",
                        "Ascoltati di recente",
                        context.getFeed(historyTracks)
                    ) else null,
                    Shelf.Category(
                        "saved",
    ...
    override suspend fun onMarkAsPlayed(details: TrackDetails) {
        ...
        // SALVA NELLA CRONOLOGIA E PULISCI
        db.playlistDao().insertHistory(
            UnifiedDatabase.HistoryEntity(
                trackId = details.track.id,
                extId = id,
                timestamp = System.currentTimeMillis(),
                data = details.track.toJson()
            )
        )
        db.playlistDao().trimHistory(150) // Memory Saver: tiene 150 tracce max
    }
