...
import dev.matteomac81888.echo.ui.feed.FeedType.Companion.toFeedType
    ...
    private suspend fun getFeedSourceData(
        ...
        val data = if (feedSortState.value != null || searchQuery != null) {
            result.mapCatching { state ->
                ...
                if (query != null) {
                    shelves = shelves.searchBy(query) { shelf ->
                        val texts = mutableListOf<String?>()
                        texts.add(shelf.title)
                        if (shelf is Shelf.Item) {
                            val media = shelf.media
                            if (media is dev.brahmkshatriya.echo.common.models.Track) {
                                texts.addAll(media.artists.map { it.name })
                                texts.add(media.album?.title ?: "")
                            }
                        }
                        texts
                    }.map { it.second }
